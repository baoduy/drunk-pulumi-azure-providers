...
All Key Vault-backed providers (`VaultSecret.ts`, `VaultCert.ts`, `VaultKey.ts`) go through `src/AzBase/KeyVaultBase.ts`, which wraps the three Key Vault SDK clients (secrets/keys/certs) behind one class per vault name (or full vault URL for sovereign clouds), auto-recovers soft-deleted items on write, and defaults to secure settings (4096-bit keys/certs, leaf-only cert key usage, key rotation on renewal, `enabled: true`). `src/AzBase/Helpers.ts` holds the shared pieces: `getCredential()` (one `DefaultAzureCredential` per process — use it instead of `new DefaultAzureCredential()`), `collect`/`searchResources` for paged SDK lists, `ignoreNotFound` for delete paths, `diffProps` for provider `diff()` implementations (inputs that identify the Azure object go in `replaceKeys`), and `waitAndRetry`, which polls for eventual consistency after a write when the client doesn't return the object directly.
